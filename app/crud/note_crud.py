...
def get_notes_by_tag(db: Session, tag_name: str, skip: int = 0, limit: int = 10):
    return db.query(models.Note).join(models.Note.tags).filter(models.Tag.name == tag_name).offset(skip).limit(
        limit).all()
